...
import io.github.panxiaochao.project.system.infrastructure.convert.ISysParamPOConvert;
import io.github.panxiaochao.project.system.infrastructure.dao.mapper.SysParamMapper;
...
public class SysParamServiceImpl implements ISysParamService, ISysParamReadModelService {
    ...
    private LambdaQueryWrapper<SysParamPO> lambdaQuery(SysParamPageQueryDTO pageQueryDto) {
        ...
        if (pageQueryDto != null) {
            // 默认按照更新时间倒序排序
            lqw.orderByDesc(SysParamPO::getUpdateAt);
            // 如果 参数名称 不为空
            if (StringUtils.isNotBlank(pageQueryDto.getParamName())) {
                lqw.like(SysParamPO::getParamName, pageQueryDto.getParamName());
            }
            // 如果 参数键 不为空
